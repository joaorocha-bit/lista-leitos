...
if df is not None:
    ...
    # CSS + Script de Auto-Ajuste de Altura
    html_style_script = """
    <style>
        body { font-family: sans-serif; margin: 0; background: white; padding: 20px; overflow: hidden; }
        .header-container { display: flex; align-items: center; margin-bottom: 20px; border-bottom: 2px solid #edf2f7; padding-bottom: 10px; }
        .titulo-painel { font-size: 28px; font-weight: bold; color: #1e293b; margin: 0; }
        .btn-print { margin-left: auto; padding: 10px 20px; background: #1e293b; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; }
        ...
        @media print { .btn-print { display: none !important; } body { padding: 0; zoom: 80%; } .coluna-fixa { position: relative !important; left: 0 !important; box-shadow: none !important; } @page { size: landscape; margin: 1cm; } }
    </style>
    <script>
        function sendHeight() {
            const height = document.getElementById('main-content').offsetHeight + 50;
    ...
    html_corpo += f"<div class='linha'><div class='coluna-fixa'><b>TOTAL GERAL</b><br><small>Total: {total_g}</small><div class='stats-container'>{stats_g}</div></div><div class='wrapper-cards'></div></div></div></div>"

    html_final = f"<html><head>{html_style_script}</head><body>{html_corpo}</body></html>"
    
    # O 'height' aqui vira apenas um valor inicial; o script dentro do HTML cuidará do ajuste real
